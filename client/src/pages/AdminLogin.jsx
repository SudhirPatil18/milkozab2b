...
import { EyeIcon, EyeSlashIcon, XCircleIcon, ExclamationTriangleIcon } from '@heroicons/react/24/outline';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Show blocked modal with admin details
        setBlockedAdmin(result.admin);
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        <p className="mt-2 text-center text-sm text-gray-600">
          Sign in to your admin account to manage the B2B platform
        </p>
      </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
            <div className="text-center">
              ...
              {blockedAdmin.blockedReason && (
                <div className="bg-yellow-50 border border-yellow-200 rounded-md p-3 mb-4">
                  <div className="flex">
                    <ExclamationTriangleIcon className="h-5 w-5 text-yellow-400" />
                    <div className="ml-3">
                      <h4 className="text-sm font-medium text-yellow-800">Reason:</h4>
                      <p className="text-sm text-yellow-700 mt-1">{blockedAdmin.blockedReason}</p>
                    </div>
                  </div>
                </div>
              )}

              {blockedAdmin.blockedBy && (
                <div className="text-xs text-gray-500 mb-4">
                  Blocked by: {blockedAdmin.blockedBy.name || 'HeadAdmin'}
                  {blockedAdmin.blockedAt && (
                    <span className="block">
                      On: {new Date(blockedAdmin.blockedAt).toLocaleDateString()}
                    </span>
                  )}
                </div>
              )}
